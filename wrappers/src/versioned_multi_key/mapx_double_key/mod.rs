    ...
    V: ValueEnDe,
{
    ...
    #[inline(always)]
    pub fn get_by_branch(&self, key: &(&K1, &K2), br_name: BranchName) -> Option<V> {
        let key = Self::encode_key(key);
        self.inner
    ...
    #[inline(always)]
    pub fn contains_key_by_branch(&self, key: &(&K1, &K2), br_name: BranchName) -> bool {
        let key = Self::encode_key(key);
    ...
    V: ValueEnDe,
{
    pub fn or_insert(self, default: V) -> ValueMut<'a, K1, K2, V> {
        let hdr = self.hdr as *mut MapxDkVs<K1, K2, V>;
