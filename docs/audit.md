...
### dagmap: "public `Orphan::get_mut()` allows cycle creation"
- **Where**: `strata/src/dagmap/raw/mod.rs`, `strata/src/basic/orphan/mod.rs`
- **Claim**: Users can mutate DagMap's parent slot to point at a descendant.
- **Reason**: The parent `Orphan` field is private and no public API exposes it. `Orphan::get_mut` is irrelevant without a handle to that field.
