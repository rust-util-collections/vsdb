...
**T5 Cache version** — one file per map (`{mpt,smt}_cache_<map_id>.bin`) stamped with the synced CommitId; in-memory sync point = (branch, commit, dirty applied). Stamp/root mismatch or diff failure → rebuild, never silent stale.
