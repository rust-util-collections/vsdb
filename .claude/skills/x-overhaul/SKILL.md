...
`$ARGUMENTS` — same scopes as `/x-review`, without `--fix`. Reject `--fix` and any
other unknown arg; never guess.
...
`.claude/docs/pragmatic-engineering.md`, `.claude/skills/x-review/SKILL.md`,
`.claude/skills/x-fix/SKILL.md`, `.claude/docs/commit-protocol.md`, and
`.claude/docs/compatibility-policy.md`; ledger before mutations. Note whether a
patch bump is owed.
