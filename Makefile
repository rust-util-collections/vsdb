...
test:
	- rm -rf ~/.vsdb /tmp/.vsdb /tmp/vsdb_testing
	cargo test --workspace --release --tests --bins --features "vs,compress" -- --test-threads=1 #--nocapture
	- rm -rf ~/.vsdb /tmp/.vsdb /tmp/vsdb_testing
	cargo test --workspace --tests --bins --features "vs" -- --test-threads=1 #--nocapture
...
testall: test
	- rm -rf ~/.vsdb /tmp/.vsdb /tmp/vsdb_testing
	cargo test --workspace --release --tests --bins --no-default-features --features "vs,rocks_engine,msgpack_codec,compress" -- --test-threads=1 #--nocapture
	- rm -rf ~/.vsdb /tmp/.vsdb /tmp/vsdb_testing
	cargo test --workspace --release --tests --bins --no-default-features --features "vs,rocks_engine,json_codec,compress" -- --test-threads=1 #--nocapture
	- rm -rf ~/.vsdb /tmp/.vsdb /tmp/vsdb_testing
	cargo test --workspace --tests --bins --no-default-features --features "vs,rocks_engine,msgpack_codec" -- --test-threads=1 #--nocapture
